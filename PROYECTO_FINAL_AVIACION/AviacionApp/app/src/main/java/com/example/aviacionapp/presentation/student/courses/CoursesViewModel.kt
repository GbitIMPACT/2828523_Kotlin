...
class CoursesViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _coursesState = MutableLiveData<Result<List<CourseItem>>>()
    val coursesState: LiveData<Result<List<CourseItem>>> = _coursesState

    private val _enrollResult = MutableLiveData<Result<Boolean>>()
    val enrollResult: LiveData<Result<Boolean>> = _enrollResult
    ...
    fun enrollInCourse(courseId: Int) {
        viewModelScope.launch {
            _enrollResult.value = Result.Loading

            try {
                val token = preferencesManager.getToken() ?: ""
                val response = apiService.enrollCourse(token, EnrollRequest(courseId))

                if (response.isSuccessful) {
                    _enrollResult.value = Result.Success(true)
                } else {
                    _enrollResult.value = Result.Error(Exception("Error al matricular"))
                }
            } catch (e: Exception) {
                _enrollResult.value = Result.Error(e)
            }
        }
    }
}